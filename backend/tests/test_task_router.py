...
def mock_async_result_factory():
    ...
    def _factory(
        state: str, result=None, ready: bool = False, failed: bool = False, info=None
    ):
        mock_res = MagicMock()
        ...
        mock_res.info = (
            info if failed else (result if isinstance(result, Exception) else None)
        )
...
def test_create_task_success(patch_celery_task_delay):
    ...
    """
    response = client.post("/api/v1/tasks", json={"user_prompt": "Test Prompt"})
...
def test_create_task_empty_prompt():
    ...
    """
    response = client.post("/api/v1/tasks", json={"user_prompt": ""})
...
def test_get_task_status_success_valid_result(
    patch_celery_async_result, mock_async_result_factory
):
    """
    ...
    mock_result = mock_async_result_factory(
        state="SUCCESS", result=valid_celery_payload, ready=True, failed=False
    )
    patch_celery_async_result.return_value = mock_result
...
def test_get_task_status_success_invalid_result_validation_error(
    ...
    invalid_celery_payload = {"status": "success", "message": "Missing file path"}
    mock_result = mock_async_result_factory(
        state="SUCCESS", result=invalid_celery_payload, ready=True, failed=False
    )

    # [CTO 修复] 模拟 Pydantic 验证失败
    # 我们 patch `TaskResultData.model_validate` 来抛出错误
    with patch(
        "app.routers.tasks.TaskResultData.model_validate",
        ...
        def failed_side_effect():
            return (
                mock_result._set_state.called
                and mock_result._set_state.call_args[0][0] == "FAILURE"
            )
...
def test_download_file_direct_not_ready(
    patch_celery_async_result, mock_async_result_factory
):
...
def test_download_file_direct_task_failed(
    patch_celery_async_result, mock_async_result_factory
):
...
def test_download_file_direct_invalid_result_format(
    patch_celery_async_result, mock_async_result_factory
):
...
def test_download_file_direct_invalid_path_relative(
    patch_celery_async_result, mock_async_result_factory
):
...
def test_download_file_direct_file_not_exist_on_disk(
    patch_celery_async_result, mock_async_result_factory
):
    ...
    with patch("app.routers.tasks.os.path.isabs", return_value=True) as mock_isabs:
        with patch(
            "app.routers.tasks.os.path.exists", return_value=False
        ) as mock_exists:
            response = client.get("/api/v1/tasks/mocked-task-id/file")
            mock_isabs.assert_called_once_with(file_path)
...
def test_download_file_direct_success(
    ...
    with patch("app.routers.tasks.os.path.isabs", return_value=True) as mock_isabs:
        with patch(
            "app.routers.tasks.os.path.exists", return_value=True
        ) as mock_exists:
            with patch(
                "app.routers.tasks.os.path.basename", return_value="real_file.pptx"
            ) as mock_basename:
